    ...
    <div class="card p-6">
      <div class="flex items-center gap-6">
        ...
        <div class="relative flex-shrink-0 group">
          <div 
            class="h-24 w-24 rounded-full overflow-hidden bg-slate-800 border-4 border-slate-700 grid place-items-center text-3xl font-bold text-violet-400 cursor-pointer"
            @click="selectAvatar"
          >
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
      ...
      <div class="card p-6">
        ...
        <form @submit.prevent="changePassword" class="space-y-4">
          ...
          <p v-if="passMessage" :class="passError ? 'text-rose-400' : 'text-emerald-400'" class="text-sm font-medium">
            {{ passMessage }}
          </p>
        </form>
      </div>
      ...
      <!-- Invite History -->
      <div class="card p-6">
        <h2 class="text-xl font-bold text-white mb-4">Board Invitations</h2>
        
        <div v-if="!invites.length" class="text-slate-400 text-sm text-center py-6">
          You haven't received any board invitations yet.
        </div>
        <div v-else class="space-y-3 max-h-72 overflow-y-auto pr-2">
          <div v-for="inv in invites" :key="inv.id" class="p-3 bg-slate-800/50 rounded-xl border border-white/5 flex items-start justify-between">
            <div>
              <div class="font-medium text-slate-200">{{ inv.board.title }}</div>
              <div class="text-xs text-slate-400 mt-1">
                Expires: {{ formatDate(inv.expiresAt) }}
              </div>
            </div>
            <div>
              <span v-if="inv.accepted" class="text-xs font-semibold text-emerald-400 bg-emerald-400/10 px-2 py-1 rounded">
                Accepted
              </span>
              <span v-else-if="new Date(inv.expiresAt) < new Date()" class="text-xs font-semibold text-rose-400 bg-rose-400/10 px-2 py-1 rounded">
                Expired
              </span>
              <span v-else class="text-xs font-semibold text-blue-400 bg-blue-400/10 px-2 py-1 rounded">
                Pending
              </span>
            </div>
          </div>
